...
function isSuperAdminRole(value: string | null | undefined): boolean {
...
function buildMessage(accion: Accion, responsable: Usuario | null, checkpoints: Checkpoint[]): string {
  ...
  const checklistText = checkpoints.length
    ? checkpoints.map((c, i) => `${c.completado ? '[x]' : '[ ]'} ${i + 1}. ${c.texto}`).join('\n')
...
Deno.serve(async (req) => {
  ...
  if (!isSuperAdminRole(currentUser.rol)) {
    return jsonResponse({ ok: false, message: 'Solo super_admin puede enviar acciones por Telegram' }, 403)
  }
