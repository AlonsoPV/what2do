...
export function KanbanHeader({
  ...
  const actionCount =
    (onNewAction ? 1 : 0) + (onToggleFilters ? 1 : 0) + (onViewModeChange ? 1 : 0)
    ...
    >
      <div className="kanban-header-title-area min-w-0 space-y-2.5">
        ...
        {actionCount > 0 ? (
          <div
            className={cn(
              'kanban-header-actions grid w-full min-w-0 gap-2 rounded-xl border border-border/70 bg-muted/25 p-2 shadow-sm ring-1 ring-border/30 sm:flex sm:items-center sm:justify-between sm:gap-3 sm:p-3',
              actionsGridClass
            )}
          >
            {onNewAction ? (
